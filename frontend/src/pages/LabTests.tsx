...
const statusColors: Record<LabOrderStatus, string> = {
  [LabOrderStatus.Ordered]: 'bg-blue-100 text-[#3B82F6]',
  [LabOrderStatus.Collected]: 'bg-purple-100 text-[#8B5CF6]',
  [LabOrderStatus.Processing]: 'bg-yellow-100 text-[#FFC107]',
  [LabOrderStatus.Completed]: 'bg-green-100 text-[#28A745]',
  [LabOrderStatus.Cancelled]: 'bg-red-100 text-[#DC3545]',
};
      ...
      <DataTable
        columns={[
          ...
          { key: 'status', header: 'Status', render: (t: LabTest) => (
            <span className={`text-xs px-2 py-0.5 rounded-full ${t.isActive ? 'bg-green-100 text-[#28A745]' : 'bg-gray-100 text-[#6C757D]'}`}>
              {t.isActive ? 'Active' : 'Inactive'}
            </span>
          )},
          { key: 'actions', header: '', render: (t: LabTest) => (
            <div className="flex gap-2 justify-end">
              <button onClick={() => openEdit(t)} className="text-xs text-[#3B82F6]">Edit</button>
              <button onClick={() => handleToggle(t)} className="text-xs text-[#6C757D]">{t.isActive ? 'Deactivate' : 'Activate'}</button>
              <button onClick={() => handleDelete(t)} className="text-xs text-[#DC3545]">Delete</button>
            </div>
          )},
        ]}
        data={tests}
      ...
      <DataTable
        columns={[
          ...
          { key: 'status', header: 'Status', render: (o: any) => (
            <span className={`text-xs px-2 py-1 rounded-full ${statusColors[o.status as LabOrderStatus] || ''}`}>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><span className="text-[#6C757D]">Status:</span>
                <span className={`ml-1 text-xs px-2 py-0.5 rounded-full ${statusColors[selectedOrder.status as LabOrderStatus] || ''}`}>
